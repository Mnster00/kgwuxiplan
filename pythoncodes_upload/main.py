...
def main():
    ...
    # 加载规划文本
    print("\n[2/6] 加载规划文本...")
    if not os.path.exists('data/guangzhou_plan.txt'):
        print("错误: data/guangzhou_plan.txt 不存在")
        return
    
    sentences = load_text('data/guangzhou_plan.txt')
    ...
    for i, sent in enumerate(sentences[:max_sentences], 1):
        print(f"   [{i}/{max_sentences}] {sent[:40]}...")
    ...
    kg_builder.save('results/knowledge_graph.json')
...
""")
    ...
    print("\n[6/6]生成可视化...")
    try:
        kg_builder.visualize('results/kg_sample.png', sample_size=30)
